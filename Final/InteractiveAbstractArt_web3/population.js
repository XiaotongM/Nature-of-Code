...
class Population {
  constructor(m, num) {
    ...
    // for (let i = 0; i < num; i++) {
    //     this.population[i] = new Face(new DNA(), i * 210, 70);
    // }
    let y;
    for (let i = 0; i < num; i++) {
      if (i % 2 == 0) {
      ...
      this.population[i] = new Face(new DNA(), 180 + floor(i / 2) * 210, y);
    }
  }
  ...
  reproduction() {
    ...
    for (let i = 0; i < this.population.length; i++) {
      ...
      // this.population[i] = new Face(new DNA(), -850 + i * 210, 70);
      let y;
      if (i % 2 == 0) {
        y = 60;
      } else {
      ...
      // this.population[i] = new Face(child, 40 + (i / 2) * 210, y);
      this.population[i] = new Face(child, 180 + floor(i / 2) * 210, y);
    }
  this.generations++;
}
